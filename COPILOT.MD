...
2. [Architectural Alignment with XGUI-3](#architectural-alignment-with-xgui-3)
3. [Overview](#overview)
4. [Project Structure](#project-structure-post-migration)
5. [Architecture](#architecture)
6. [Animation Framework](#animation-framework)
7. [Recent Improvements (Current Session)](#recent-improvements-current-session)
8. [Recent Improvements (Previous Session)](#recent-improvements-previous-session)
...
16. [Migration to MAUI](#migration-to-maui-blazor-hybrid-and-web-app-november-2025)
17. [Bug Fixes](#bug-fixes)
...
This project is the **iOS equivalent of [XGUI-3](https://github.com/Xenthio/XGUI-3)** - a FakeOS simulator that replicates iOS instead of Windows. While XGUI-3 recreates the Windows experience in s&box, OldiOSExperience recreates iOS 5 using Blazor.

### Project Roadmap

- [x] Layout Designer equivalent (UIKit components library)
- [x] Core system services (SpringBoard, BackgroundAppManager)
- [x] App lifecycle management
- [x] Lock screen with slide-to-unlock
- [x] App switcher (multitasking)
- [x] 22+ built-in apps
- [ ] Resource files for themes/customization
- [ ] Notification Center
- [ ] Control Center (iOS 7+ style option)
- [ ] iCloud simulation / data persistence
- [ ] App installation simulation
- [ ] Push notification simulation

### Key Parallels with XGUI-3

Both projects share the same goals:
- Authentic replication of an operating system (Windows for XGUI-3, iOS for OldiOSExperience)
- Component-based UI framework (XGUI components ≈ UIKit components)
- Service-oriented architecture mimicking real OS services
- Support for multiple apps/programs with lifecycle management

---

## Architectural Alignment with XGUI-3

This project follows the same **architectural philosophy** as [xgui-3_test](https://github.com/Xenthio/xgui-3_test) - both are FakeOS simulators that authentically replicate an operating system. While XGUI-3 replicates Windows, OldiOSExperience replicates iOS.

### Conceptual Mapping

The core concepts are the same, just with iOS-appropriate naming. Paths shown are relative to `OldiOS.Shared/`:

| Windows Concept (XGUI-3) | iOS Concept (OldiOSExperience) | Location |
|--------------------------|--------------------------------|----------|
| ProcessManager | BackgroundAppManager | CoreServices/ |
| BaseProcess | AppState | Models/ |
| Shell/Explorer | Springboard | Apps/Springboard/ |
| Programs | Apps | Apps/ |
| ControlPanel | Settings App | Apps/Settings/ |
| Logon Screen | Lock Screen | Apps/Springboard/Components/ |
| FakeExecutable | FakeApp | CoreServices/ |
| ProgramDescriptor | AppDescriptor | CoreServices/ |
| FileSystem | VirtualFileSystem | CoreServices/FileSystem/ |
| Registry | UserDefaults (Future) | CoreServices/ |
| MessageBox | UIAlertController (Future) | Apps/UIKit/ |

### Core Services (CoreServices/)

The `CoreServices` folder contains the fundamental system services:

- **AnimationService** - Coordinated animations for app transitions
- **BackgroundAppManager** - App lifecycle and multitasking
- **BatteryService** - Battery state simulation
- **DisplaySettings** - Screen resolution and scaling
- **GestureService** - Touch/gesture recognition
- **SpringboardService** - Home screen layout and app organization
- **AppDescriptor** - iOS equivalent of ProgramDescriptor for .app bundles
- **FakeApp** - Creates and reads .app bundles (like FakeExecutable for .exe)
- **SystemSetup** - Initializes the file system with all apps

### Virtual File System (CoreServices/FileSystem/)

Implements iOS-like sandboxed file system:

```
/                           # Root
├── Applications/           # System apps (.app bundles)
├── System/
│   └── Library/
│       ├── CoreServices/
│       │   └── SpringBoard.app/
...
└── var/
    └── mobile/
        ├── Applications/   # User apps and sandboxes
        ├── Library/
        │   └── Preferences/    # UserDefaults storage
        ├── Documents/
        └── Media/
            ├── DCIM/       # Camera roll
            └── iTunes_Control/
...
### Design Principles (Shared with XGUI-3)

1. **Authentic Replication**: Faithfully recreate the original OS experience
2. **Component-Based Architecture**: Modular, reusable UI components (UIKit ≈ XGUI)
3. **Service Layer**: System services mirror real OS daemons/services
4. **Event-Driven**: Use events for inter-component communication
5. **Platform Abstraction**: Support multiple platforms (Web, MAUI native)

### Future Enhancements (Aligned with XGUI-3 Goals)

- [x] Add `CoreServices` folder for core system services
- [x] Move Springboard to Apps/ (it's an app like Explorer is in Windows)
- [x] Add `VirtualFileSystem` (iOS equivalent of FileSystem)
- [x] Add `FakeApp` for .app bundles (iOS equivalent of FakeExecutable for .exe)
- [x] Add `AppDescriptor` (iOS equivalent of ProgramDescriptor)
- [x] Add `SystemSetup` to initialize file system with apps
- [ ] Add `UserDefaultsService` for settings persistence
- [ ] Add `NotificationService` for push notifications
- [ ] Add theme/resource system for customization
...
## Music App Improvements (November 2025)

**Date:** November 20, 2025

**Task:** Flesh out the Music app to make it look correct and fully usable like the Settings app.

**Implementation Complete:** ✅

### Changes Made

**1. WebMediaLibraryService with Mock Data**
- Created WebMediaLibraryService.cs for web platform
- Added 5 classic rock albums with authentic metadata:
  - The Beatles - Abbey Road (7 songs)
  - Pink Floyd - The Dark Side of the Moon (7 songs)
  - Queen - A Night at the Opera (7 songs)
  - Led Zeppelin - Led Zeppelin IV (6 songs)
  - The Rolling Stones - Sticky Fingers (5 songs)
- Added 2 sample playlists (Favorites, Rock Classics)
- Total: 32 mock songs, 5 albums, 5 artists
- Registered service in OldiOS.Web/Program.cs

**2. Complete Navigation Structure**
Created dedicated page components for all 5 tabs:
- **PlaylistsRoot.razor** - Shows all playlists with song counts
- **ArtistsRoot.razor** - Lists all artists with album counts
- **ArtistPage.razor** - Artist detail with albums and "All Songs" option
- **AlbumsRoot.razor** - Album listing with "Cover Flow" button
- **SongsRoot.razor** - All songs with "Now Playing" button
- **MoreRoot.razor** - Additional options (Genres, Composers, Compilations)
- All pages use ViewNavigator for proper navigation stack

**3. Consistent iOS 5 Music App Styling**
Created/updated 9 CSS files for dark theme consistency:
- PlaylistsRoot.razor.css
- ArtistsRoot.razor.css
- ArtistPage.razor.css
- SongsRoot.razor.css
- AlbumsRoot.razor.css
- AlbumPage.razor.css
- MoreRoot.razor.css
- NowPlayingPage.razor.css (enhanced with better controls)
- MusicApp.razor.css (cleaned up duplicates)

All views now use consistent #000 background matching iOS 5 Music app

**4. Empty States Throughout**
Added proper empty states to all views:
- Playlists tab - Shows "No Playlists" with icon
- Artists tab - Shows "No Artists" with icon
- Songs tab - Shows "No Songs" with icon
- Albums tab - Shows "No Albums" with icon
- Album detail - Shows "No Songs" with icon
- All use consistent styling with 120px icons and proper spacing

**5. Enhanced Now Playing Screen**
- Improved album art display with shadow effect
- Better styled playback controls (Previous, Play/Pause, Next)
- Proper progress bar with time labels
- Volume slider with speaker icons
- Gradient background matching iOS 5
- Proper spacing and layout

**6. Integration with Existing Systems**
- MusicApp.razor now loads playlists from service
- All tabs properly integrated with ViewNavigator
- Tab bar navigation works correctly
- Cover Flow integration maintained
- Playback state management preserved

### Files Created (13 new files)
- OldiOS.Web/WebMediaLibraryService.cs
- Apps/Music/Pages/PlaylistsRoot.razor
- Apps/Music/Pages/PlaylistsRoot.razor.css
- Apps/Music/Pages/ArtistsRoot.razor
- Apps/Music/Pages/ArtistsRoot.razor.css
- Apps/Music/Pages/ArtistPage.razor
- Apps/Music/Pages/ArtistPage.razor.css
- Apps/Music/Pages/SongsRoot.razor.css
- Apps/Music/Pages/AlbumsRoot.razor.css
- Apps/Music/Pages/AlbumPage.razor.css
- Apps/Music/Pages/MoreRoot.razor
- Apps/Music/Pages/MoreRoot.razor.css
- Apps/Music/Pages/NowPlayingPage.razor.css

### Files Modified (4 files)
- OldiOS.Web/Program.cs - Registered IMediaLibraryService
- Apps/Music/MusicApp.razor - Added Playlists property, updated tab routing, cleaned up
- Apps/Music/MusicApp.razor.css - Removed duplicates, improved styling
- Apps/Music/Pages/SongsRoot.razor - Added empty state
- Apps/Music/Pages/AlbumsRoot.razor - Added empty state
- Apps/Music/Pages/AlbumPage.razor - Added empty state

### Quality Comparison with Settings App
The Music app now matches Settings app quality in all areas:
- ✅ ViewNavigator usage - Consistent navigation patterns
- ✅ UIKit component integration - Proper use of UINavigationBar, UITableView, UITableViewCell
- ✅ Navigation patterns - Back buttons, navigation stack, tab switching
- ✅ Consistent styling - Dark theme throughout, proper spacing
- ✅ Empty states - All views handle empty data gracefully
- ✅ Mock data service - WebMediaLibraryService provides demo content
- ✅ Code organization - Clean separation of concerns, proper component structure

### Build Status
✅ **Build successful** - 0 errors, 7 warnings (pre-existing, unrelated to Music app)
✅ **CodeQL scan** - 0 security alerts

### Remaining Optional Enhancements
The following are optional improvements that could be made in the future:
- Mini player bar at bottom (persistent playback controls)
- Actual audio playback (requires real audio files)
- Cover Flow enhancements (already functional)
- More tab full implementation (Genres, Composers, Compilations details)
